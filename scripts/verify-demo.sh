...
curl -s "$BASE_URL/client/sales-narrative" | python3 -m json.tool | head -80

echo
echo "---- client demo agenda ----"
curl -s "$BASE_URL/client/demo-agenda" | python3 -m json.tool | head -80

echo
echo "---- client follow-up email ----"
curl -s "$BASE_URL/client/follow-up-email" | python3 -m json.tool | head -100

echo
echo "---- client buyer faq ----"
curl -s "$BASE_URL/client/buyer-faq" | python3 -m json.tool | head -100
...
curl -s "$BASE_URL/client/validation-matrix" | python3 -m json.tool | head -120
